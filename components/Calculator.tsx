...
function SliderField({ label, value, min, max, step, onChange, display, inputSuffix }: {
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between text-sm">
        ...
        <div className="flex items-center gap-1.5">
          <input
            ...
            className="w-20 rounded-lg border border-slate-200 px-2.5 py-1.5 text-right text-sm font-semibold text-brand-600 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-100"
          />
          {inputSuffix && <span className="text-xs text-slate-400">{inputSuffix}</span>}
        </div>
      </div>
      <input
        type="range"
      ...
      />
      <p className="mt-1.5 text-xs text-slate-400">{display}</p>
    </div>
  );
}
  ...
  return (
    <section className="mx-auto max-w-5xl px-6 py-24">
      ...
      >
        ...
        <p className="mt-3 text-slate-600">Aucune inscription nécessaire. Bougez les curseurs, le prix se calcule en direct.</p>
      </motion.div>
        ...
        >
          ...
          >
            ...
            <motion.p
              ...
              transition={{ duration: 0.4 }}
              className="text-4xl font-extrabold text-brand-700"
            >
              {formatEuro(stats.monthly)}<span className="text-lg font-medium text-slate-400">/mois</span>
            </motion.p>
            ...
            <div className="mt-4 border-t border-brand-200 pt-4">
              <Row label="Gain vs perte churn" value={`${formatEuro(stats.savingsPerMonth)}/mois`} bold valueClassName="text-emerald-600" />
            </div>
            <a
              href="/signup"
              className="mt-6 flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-brand-600/20 transition hover:bg-brand-700"
            >
          ...
          <motion.div
            ...
            className="rounded-3xl border border-red-100 bg-red-50/60 p-6"
          >
            <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-red-500">Sans Churnly</p>
            ...
            <div className="mt-3 border-t border-red-200 pt-3">
              <Row label="Perte annuelle" value={formatEuro(stats.annualLoss)} bold valueClassName="text-red-600" />
            </div>
          </motion.div>
          ...
          >
            <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-emerald-600">Avec Churnly (-50% churn)</p>
            ...
            <div className="mt-3 border-t border-emerald-200 pt-3">
              <Row label="Économies annuelles" value={formatEuro(stats.annualSavings)} bold valueClassName="text-emerald-600" />
