...
export default function SectionToc({ items }: { items: TocItem[] }) {
  ...
  return (
    <>
      ...
      {/* Équivalent mobile : juste les points, jamais le texte — assez de
          place pour naviguer sans étaler du texte sur un petit écran. */}
      <nav
        aria-label={dict.common.tocLabel}
        className="fixed right-2.5 top-1/2 z-40 -translate-y-1/2 lg:hidden"
      >
              ...
              >
                <span
                  className={`rounded-full transition-all duration-300 ${
                    active ? 'h-2 w-2 bg-brand-500' : 'h-1.5 w-1.5 bg-slate-300 dark:bg-slate-700'
                  }`}
                />
              </button>
            );
