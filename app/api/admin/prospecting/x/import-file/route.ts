...
import { extractXLeadsFromRawText, parseStructuredXBlocks, type ExtractedXLead } from '@/lib/xProspectingDraft';
...
// Enlève aussi la query string et le fragment, et unifie twitter.com vers
// x.com — même raison que normalizeXUrl dans route.ts : sans ça, deux
// liens vers le même profil (avec des paramètres différents, ou l'un en
// twitter.com et l'autre en x.com) ne seraient pas reconnus comme doublons.
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error('[prospecting/x/import-file] unexpected error', err);
...
async function handleImport(req: NextRequest) {
  ...
  // voir parseStructuredXBlocks) : fiable, gratuit, et sans le risque de
  // troncature d'un appel IA sur un gros lot de messages longs. Seuls les
  // fichiers où ça ne trouve rien passent par l'extraction IA (texte brut
  // non structuré).
  const needsAiExtraction: string[] = [];
  const allLeads: ExtractedXLead[] = [];
  for (const rawText of rawTexts) {
    const structured = parseStructuredXBlocks(rawText);
  ...
  for (let i = 0; i < needsAiExtraction.length; i += EXTRACT_CONCURRENCY) {
    ...
    const results = await Promise.all(chunk.map((t) => extractXLeadsFromRawText(t).catch(() => [] as ExtractedXLead[])));
  ...
  const deduped = allLeads.filter((lead) => {
    const key = normalizeXUrl(lead.xUrl);
  ...
  if (deduped.length === 0) {
    return NextResponse.json({ added: 0, skippedDuplicate: 0, message: 'Aucun profil X exploitable (nom + lien + message) trouvé dans ces fichiers.' });
  }

  // Ne jamais recontacter une personne déjà dans la file (en attente ou déjà
  // marquée envoyée) — même principe que la prospection email/LinkedIn/Instagram.
  const { data: existing, error: existingError } = await auth.supabaseAdmin
    .from('x_prospecting')
    .select('x_url');
  if (existingError) {
    console.error('[prospecting/x/import-file] existing lookup failed', existingError);
    return NextResponse.json(
      { error: isMissingTableError(existingError) ? missingTableMessage(MIGRATION_FILE) : 'Lecture de la file existante échouée.' },
      { status: 500 },
    );
  }
  const existingUrls = new Set((existing ?? []).map((e) => normalizeXUrl(e.x_url)));

  const newLeads = deduped.filter((lead) => !existingUrls.has(normalizeXUrl(lead.xUrl)));
  ...
  const { error: insertError } = await auth.supabaseAdmin.from('x_prospecting').insert(toInsert);
  if (insertError) {
    console.error('[prospecting/x/import-file] insert failed', insertError);
    return NextResponse.json(
