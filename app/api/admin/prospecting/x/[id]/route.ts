...
// raisonnement (sinon "notx.com/y" passerait la vérification).
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (error) {
    console.error('[prospecting/x/id] update failed', error);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (error) {
    console.error('[prospecting/x/id] delete failed', error);
    return NextResponse.json(
