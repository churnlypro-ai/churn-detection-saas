...
export default function AdminCloserProspectsPage() {
  ...
  const loadCount = useCallback(async (authToken: string) => {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      const status = await loadCount(authToken);
      if (status === 'forbidden') setForbidden(true);
      setLoading(false);
    });
  }, [router, getAuthToken, loadCount]);
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl px-6 py-12">
        ...
        <h1 className="mt-4 mb-2 text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Import de prospects (cold call)</h1>
        <p className="mb-8 text-sm text-slate-500 dark:text-slate-400">
          Alimente la file &laquo;&nbsp;à appeler&nbsp;&raquo; du dashboard closer (<Link href="/closer" className="text-brand-600 hover:underline dark:text-brand-400">/closer</Link>) en une fois, depuis un export d&apos;annuaire d&apos;entreprises (Pappers, data.gouv.fr...) ou Apollo.
        </p>

        {!loading && (
          <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <p className="mb-2 text-xs text-slate-500 dark:text-slate-400">
              Colonnes reconnues (peu importe l&apos;ordre, en-têtes en français ou anglais) : <code className="rounded bg-slate-100 px-1 py-0.5 dark:bg-slate-800">name</code>/<code className="rounded bg-slate-100 px-1 py-0.5 dark:bg-slate-800">nom</code>,{' '}
            ...
            <p className="mb-4 text-xs text-slate-400 dark:text-slate-500">
              Toute ligne sans numéro exploitable (moins de 8 chiffres — &laquo;&nbsp;à confirmer&nbsp;&raquo;, vide...) est automatiquement ignorée. Les doublons (même numéro déjà dans la file) aussi.
            </p>
            <label className="flex w-full cursor-pointer flex-col items-center gap-2 rounded-xl border-2 border-dashed border-slate-200 bg-slate-50/60 px-6 py-10 text-center transition hover:border-brand-300 hover:bg-brand-50/40 dark:border-slate-700 dark:bg-slate-950 dark:hover:border-brand-700">
              ...
              <span className="text-xs text-slate-400 dark:text-slate-500">.csv, .xlsx, .xls — un seul fichier, jusqu&apos;à plusieurs milliers de lignes</span>
              <input
                type="file"
                accept=".xlsx,.xls,.csv"
                className="hidden"
